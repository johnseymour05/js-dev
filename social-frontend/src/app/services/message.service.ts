...
export class MessageService {
    ...
    createPost(post: Post): Observable<Post> {
        return this.http.post<Post>('http://localhost:8080/Social/all/createPost', post);
    }
    getUserPosts(): Observable<Post[]>  {
        const user = JSON.parse(sessionStorage.getItem('currentUser'));
        return this.http.post<Post[]>('http://localhost:8080/Social/all/getUserPosts', user);
    }
    getAllPosts(): Observable<Post[]> {
        return this.http.get<Post[]>('http://localhost:8080/Social/all/allPosts');
    }
    // // create a post
